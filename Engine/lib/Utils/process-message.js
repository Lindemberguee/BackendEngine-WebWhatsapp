    ...
    if (protocolMsg) {
        ...
        switch (message.messageStubType) {
            case Types_1.WAMessageStubType.GROUP_PARTICIPANT_CHANGE_NUMBER:
                participants = message.messageStubParameters.map((a) => { try { return JSON.parse(a); } catch { return a; } }) || []
            ...
            case Types_1.WAMessageStubType.GROUP_MEMBERSHIP_JOIN_APPROVAL_REQUEST_NON_ADMIN_ADD:
                let participant = message.messageStubParameters?.[0];
                try { participant = JSON.parse(participant); } catch { }
